...
async fn test_domain_object_first_name() {
    ...
    let row = db
        .client()
        .await
        .query_one("SELECT 'タナカ', 'some'", &[])
        .await
        .unwrap();
    let name = row.try_get::<_, NameJp>(0).unwrap();
    assert_eq!(name, NameJp("タナカ".to_string()));

    // set test
    let name = NameJp("タナカ".to_string());
    let row = db
        .client()
        .await
    ...
    assert_eq!(test.test, NameJp("タナカ".to_string()));
